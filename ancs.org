...
!The function \ty{FindHomologies()} accepts query and subject
!sequences, an enhanced suffix array of the subject, and the minimum
!anchor length \ty{a}. The function returns a slice of segments
!(homologies) and a bool map of segregation sites.
...
'remote' match somewhere else. There are two functions to write for
these scenarios. In the first case, we try to find the longest common
prefix at the given positions in the query and the subject using the
function \ty{anchorLcp()}. In the second case, we turn to the ESA and
look for a match using the function \ty{anchorEsa()}. Both functions
advance in the subject and update \ty{currStartS} and \ty{currLen},
both functions return \ty{true} if a match is found.

We put both functions into a short-circuit evaluation, that is,
\ty{anchorEsa} is called only if \ty{anchorLcp} has returned
\ty{true}. If a match is found, we proceed with calculating the end
...
#+end_src
#+begin_export latex
The function \ty{anchorLcp()} accepts pointers to length of the
...
the subject. The function returns a boolean and updates the start and
the length of the current match.

We calculate the size of a gap between the current and previous
position in the query, a position in the subject to try to match the
longest common prefix (LCP) with. If the this position is beyound the
end of the subject or if the gap is longer than the minimum anchor
length (that is, not random), we return \ty{false} early. If not so,
we update the current position in the subject and find the LCP at this
position. Then we update the length of the current match with the
length of the LCP. Should the latter satisfy the minimum anchor length
threshold, we return \ty{true}.
#+end_export
#+begin_src go <<Functions>>=
...
#+end_src
#+begin_export latex
The function \ty{lcp} accepts and returns length of the longest common
prefix between two slices of bytes.
#+end_export
#+begin_src go <<Functions>>=
...
#+end_src
#+begin_export latex
The function \ty{anchorEsa()} accepts pointers to length of the
current match and its start in the subject, a map of segregation
sites, minimum anchor length, current query prefix, and a pointer to
