...
!The function \ty{FindHomologies} accepts a query sequence, an
!enhanced suffix array of the subject, and the minimum anchor length
!\ty{a}. The function returns a slice of segments (homologous regions,
!or homologies) and a bool map of segregation sites found within the
!homologies. If no homologies have been found, an empty slice of
!segments and empty map of segregation sites are returned.
...
!5) the current query prefix; 6) a pointer to subject's ESA. The
...
#+end_src
#+begin_export latex
\subsubsection{\ty{Intersect}}
!The function \ty{Intersect} accept a slice of segments and finds
!coordinates of intersecting segments, which cover a given fraction of
!target genomes.

We interpret the minimum fraction of intersecting genomes. Thereafter,
we calculate pile heights, that is, how many times a nucleotide of the
subject is covered by piled up homologous regions. Then we convert
pile heights to actual segments.
#+end_export
#+begin_src go <<Functions>>=
  func Intersect(h []Seg, g int, f float64, slen int) []Seg {
	  //<<Interpret the threshold fraction>>
	  p := pileHeights(h, slen)
	  //<<Convert pile heights to segments>>
  }
#+end_src
#+begin_export latex
We calculate the product of the threshold fraction $f$ and the total
number of queries $g$, that is, the total number of genomes taken into
the analysis \textit{excluding} the implied subject. We round the
product down to the nearest integer, but if the latter happens to be
zero, we set the value to 1. Thus, we will be always trying to find
the partial intersection for any $0 \le f < 1$ and $g /ge 1 $.
#+end_export
...
adjacency'. This map holds ends of the left segment from a pair. Then
we perform the conversion and return the segments that make up the
intersection.
#+end_export
#+begin_src go <<Convert pile heights to segments>>=
  //<<Create a map of adjacency>>
  intersection := pileToSeg(p, t, isAdj)
  return intersection
#+end_src
#+begin_export latex
We search for such pairs of segments, where the end of the left
segment is immideately followed by the start of the right segment. In
terms of 0-based coordinates, this implies that the start and the end
...
#+end_export
#+begin_src go <<Create a map of adjacency>>=
