...
!The function \ty{Intersect} finds interection segment coordinates,
!which cover a given fraction of target genomes.

We interpret the minimum fraction of intersecting genomes. Thereafter,
we calculate pile heights and filter the pile using the threshold.

//I was going to repdroduce the exact behavoir of the matching as
implemented in phylonium. Sometimes a contigious region is split in
two parts for some reason to be investigated. During the
pile-height-based intersection, the adjanced regions can be again
merged into one. To avoid this, we would remember the ends of the
regions and use them to enforce the original structure of homologous
regions. For now, I comment it out//.
#+end_export
#+begin_src go <<Functions>>=
...
keys and values of the pile, close or extend opened segments, and open
new segments depending on the pile heights at given positions.
#+end_export
#+begin_src go <<Functions>>=
