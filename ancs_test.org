...
\begin{table}[H]
  ...
  \caption{Test inputs for \ty{findHomologs}. $S$, subject, $Q$,
    query, $c1$ and $c2$, contigs 1 and 2, $N$, substitutions}
...
by \ty{phylonium} on the cases from Table~\ref{tab:testCasesFH}.
#+end_export
...
#+end_export
#+begin_export latex
We initialize and populate a \ty{query} struct. We read all sequences
...
\end{figure}
We initialize a test \ty{Homologs}. Then we test \ty{pileHeights},
