...
\begin{table}[H]
  \centering
  \caption{Test inputs for \ty{FindHomologies}. $S$, subject, $Q$, query, $c1$ and $c2$, contigs 1 and 2}
  \begin{tabular}{l l l l l}
    ...
    \hline
    \ty{mutations}&$c1[500:1000]$&$c1[500:1000]$, substitutions $16,17,66,291,387$\\
  \end{tabular}
...
our own function. We read \ty{fasta} files produced by \ty{phylonium}
on the cases from Table~\ref{tabTestCasesFH}.
#+end_export
...
#+end_src
#+begin_export latex
The function \ty{prepareSubject} returns the length and GC content of
the sequence, the minimum anchor length, and ESA of the subject.
#+end_export
#+begin_src go <<Auxillary functions>>=
