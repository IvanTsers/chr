...
#+end_src
#+begin_export latex
We extract only visited elements from \ty{segs} and set the new value of
\ty{h.S}.
#+end_export
...
calculate the new coordinates before calling \ty{Intersect}. Thusm we
expect the new start coordinates to be included in the headers of the
subsetted reference in this format:
\begin{verbatim}
>subsetContigName$x
\end{verbatim}
where \ty{x} is the start of the contig in the original reference. We
use the dollar sign \ty{\$} to safely delimit the name and the
coordinate. Thus, \textbf{if you plan to use \ty{Intersect} on a
  subsetted reference but want the output to show the original
  coordinates, consider adding the suffix \ty{\$x} (zero-based) to the
  headers of the pre-processed sequences.} These will be recognized by
\ty{Intersect} when \ty{ShiftRefRight} is enabled.
#+end_export
#+begin_export latex
...
The function \ty{extractShiftField} accepts pointers to a header
and the \ty{subject} struct. It is supposed to update the
\ty{subject.contigShifts} map if the input header contains the shift
field \ty{\$x} (Section~\ref{Shifts}). The function's only return is
an error.

We initialize an error, then check whether our input variables are
\ty{nil}. Then we create a regular expression, parse the header, and
create error messages if there are problems with the parsing. Once we
have read the shift field, we remove it from the header and update the
shifts map of the \ty{subject} struct.
#+end_export
#+begin_src go <<Functions>>=
  func extractShiftField(header *string, subject *subject) error {
	  var err error
	  //<<Are the inputs \ty{nil}?>>
	  shift := 0
...
#+end_src
#+begin_export latex
We import \ty{regexp} and \ty{strconv}.
#+end_export
#+begin_src go <<Imports>>=
  "regexp"
  "strconv"
#+end_src
#+begin_export latex
We check whether the header or the subject are \ty{nil}. If so,
we create the corresponding error message and return early.
#+end_export
#+begin_src go <<Are the inputs \ty{nil}?>>=
  if header == nil {
...
#+end_src
#+begin_export latex
We create an error message for an empty match.
#+end_export
#+begin_src go <<Error of matching>>=
  err = fmt.Errorf(
...
We initialize variables to operate with during the search for
homologs, conduct the search, reduce overlaps, and return the
...
that starts right after the last mismatch, 2) there is a distant match
somewhere else in the ESA. There are two functions to be written.
...
#+end_src
#+begin_export latex
We decide whether the next byte is going to be a nucleotide of the
subject or an \ty{N}.
#+end_export
...
header \ty{ch}, a tab, and coordianates of the start (\ty{cs}) and the
end (\ty{ce}):
\begin{verbatim}
ch\t(cs..ce)
\end{verbatim}
...
\ty{subject}. It finds the subject contig, to which the segment
belongs, and returns its header and the coordinates of the segment on
the contig.

We initialize the output variables and 'unpack' some fields of the
\ty{subject}. Then we check whether our segment is located within some
of the contigs using the function \ty{isWithin}, which we still need
to write. If so, we get its header. Then we get the segment's start
and shift it using the map of shifts. We remember that we were adding
one to the intermediate total length of the subject to consider the
sentinel exclamation marks inserted between the concatenated contigs
(Section~\ref{Prepare the subject}). Thus, we have to subtract it back
if we're dealig with any contig starting from the second. Once we have
the shift value at hand, we can calculate the end. A segment can be
found only once because the homologs are unique and do not
overlap. Thus, we break as soon as we've found the contig it belongs
to.
#+end_export
#+begin_src go <<Functions>>=
...
\ty{k}. Then we append these positions to our \ty{segSiteStr}.
#+end_export
...
#+end_src
#+begin_export latex
We traverse the segregating sites of the current segment, find
existing keys, and save the coordinate in a slice.
#+end_export
...
#+end_src
#+begin_export latex
Before printing the output, we chose between one- and zero-based
coordinate system.
#+end_export
#+begin_src go <<Append a segsite>>=
