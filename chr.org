...
!5) a switch to clean* subject sequence;
!6) a switch to clean* query sequences;
!7) a switch to print positions of segregating sites in output headers;
...
data. We append the header to the slice of contig headers. Then we
append an exclamation mark to the combined subject data and increment
the total length by one. The new total length gives us the start of
the contig on the concatenated subject data. We create a \ty{seg} and
append it to the slice of contig segments. Finally, we append the
contig's data to the combined subject's data and increment the total
length.
#+end_export
...
header \ty{ch}, as well as the start (\ty{cs}) and the end (\ty{ce})
coordinates on it:
\begin{verbatim}
ch\t(cs..ce)
\end{verbatim}
To find those, we call \ty{findSegment}, which we still have to
implement. We convert zero-based end-exclusive coordinates to
one-based end-exclusive coordinates if the respective switch is set to
\ty{true}. If \ty{printSegSitePos} has been switched to \ty{true}, we
build segsite positions string with the \ty{buildSegSiteStr} function,
which is yet to be defined. Then we append the string to the header.
#+end_export
...
\ty{subject}. Then we check whether our segment is within some of the
contigs using the function \ty{isWithin}, which we still need to
write. If so, we get its header and get the segment's start and end
coordinates. A segment can be found only once, so we break as soon as
its contig is identified.
#+end_export
#+begin_src go <<Functions>>=
