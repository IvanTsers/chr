...
sequences from it, concatenate and normalize them. Then we set the values
...
segments of the reference. If the reference has been normalized, two
adjacent homologous segments can be merged into one. The normalization
implies 1) converting all characters to uppercase, 2) removal of
non-conventioonal nucleotides, that is, keeping only \ty{A, T, G,
...
sequence. The function returns a slice of pointers to fasta entries
(type \ty{fasta.Sequence}).
...
coordinates if the respective switch is set to \ty{true}.
#+end_export
...
#+end_src
#+begin_export latex
\subsection{Function \ty{PrintSegsiteRanges}}
...
#+begin_src go <<Functions>>=
  func PrintSegsiteRanges(h Homologs, file *os.File, printOneBased bool) {
	  ...
	  if len(ns) == 0 {
		  ...
		  for _, seg := range segs {
			  //<<Print segsites found within a homolog>>
		  }
	  }
  }
#+end_src
...
#+end_export
#+begin_src go <<Print segsites found within a homolog>>=
  k := []int{-1}
