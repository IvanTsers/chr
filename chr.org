...
#+end_src
#+begin_export latex
We normalize the subject contigs, that is, remove non-canonical
nucleotides and convert nucleotides to uppercase.
#+end_export
#+begin_src go <<Normalize subject contigs>>=
  for _, s := range r {
	  fastautils.Clean(s)
...
path to a query file, read all sequences from it, normalize and
concatenate them. Then we set the values for the \ty{seq} and \ty{l}
fields.
#+end_export
...
#+end_src
#+begin_export latex
We normalize the query, that is, remove non-canonical nucleotides and
convert nucleotides to uppercase.
#+end_export
#+begin_src go <<Normalize query contigs>>=
...
#+end_src
#+begin_export latex
We print the error of sequence concatenation and exit.
#+end_export
