...
The function \ty{extractShiftField} accepts pointers to a header and
the \ty{subject} struct. It updates the \ty{subject.contigShifts} map
if the input header contains the shift field \ty{\$x}
...
(if necessary) and append it to the slice of homologies. We apply the
filter for the maximum fraction of mismatches.
#+end_export
