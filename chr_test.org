...
\ty{sort.Ints} function from the default \ty{Go} package. Then we
...
#+end_src
#+begin_export latex
\subsection{\ty{Homologs.reduceOverlaps}}
We create a number of segments to test the method \ty{reduceOverlaps},
...
#+end_src
#+begin_export latex
We test \ty{findHomologs} on input sequences covering various cases
...
initialize the struct, read the contigs, normalize subject's contigs,
process them, calculate the minimum anchor length, build an ESA, and
populate the struct.
#+end_export
#+begin_src go <<Auxillary functions>>=
...
#+end_src
#+begin_export latex
We calculate GC content in the subject and use it to find the
...
from it, concatenate and clean them. Then we set the values for the
...
homologs, sort them and reduce overlaps. Then we convert \ty{Seg}
...
for which a number of homologies was found in five query sequences
(Figure~\ref{fig:testCasesPile}).
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{figTestCasesPile.eps}
...
#+end_src
#+begin_export latex
We define the test cases for \ty{pileToSeg}. These include fair
sensitivity thresholds from 20\% to 80\%, as well as some edge
cases. The test cases corresspond to Figure~\ref{fig:testCasesPile}.
#+end_export
...
We emulate a real case workflow using the \ty{chr} package
functions. There are input genomes we want to intersect. 
...
#+end_src
#+begin_export latex
We read the reference. Note: we always use the file \ty{t1.fasta} as
the reference. We set \ty{Parameters} and call \ty{Intersect} with
those \ty{Parameters}. We compare the results to the output of
\ty{phylonium -p}. As \ty{phylonium} can only return complete
intersections, we will use \ty{Intersect} with the 100\% sensitivity
threshold.
#+end_export
